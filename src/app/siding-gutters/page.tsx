import { Metadata } from 'next';
import { searchPlaces } from '@/utils/googlePlaces';
import ContractorListings from '@/components/ContractorListings';
import InquiryForm from '@/components/InquiryForm';

export const metadata: Metadata = {
  title: 'Top Siding & Gutter Contractors in Denver',
  description: 'Find professional siding and gutter contractors in Denver for installation, repair, and maintenance services.',
  openGraph: {
    title: 'Top Siding & Gutter Contractors in Denver',
    description: 'Find professional siding and gutter contractors in Denver for installation, repair, and maintenance services.',
    url: 'https://www.topcontractorsdenver.com/siding-gutters',
    siteName: 'Denver Contractors',
    locale: 'en_US',
    type: 'website',
  },
};

async function getSidingGutterContractors() {
  try {
    const response = await searchPlaces('siding and gutter contractors', 'Denver, CO');
    if (!response || !response.results) {
      throw new Error('Invalid response from searchPlaces API');
    }
    return response.results;
  } catch (error) {
    console.error('Error fetching siding & gutter contractors:', error);
    throw error;
  }
}

export default async function SidingGuttersPage() {
  try {
    const contractors = await getSidingGutterContractors();

    return (
      <div className="min-h-screen bg-gray-50">
        {/* Hero Section */}
        <div className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900 text-white">
          <div className="container mx-auto px-4 py-16">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 text-center">
              Denver Siding & Gutter Contractors
            </h1>
            <p className="text-xl text-center text-blue-100 max-w-3xl mx-auto">
              Professional siding installation and gutter services for your home
            </p>
          </div>
        </div>

        {/* Main Content */}
        <div className="container mx-auto px-4 py-12">
          {/* Contractor Listings */}
          <div className="mb-16">
            <h2 className="text-3xl font-bold text-gray-900 mb-8">
              Featured Siding & Gutter Contractors in Denver
            </h2>
            <ContractorListings contractors={contractors} />
          </div>

          {/* Contact Section */}
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-2xl mx-auto">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-gray-900 mb-4">
                Get a Free Consultation
              </h2>
              <p className="text-lg text-gray-600 mb-4">
                Connect with top siding and gutter contractors in Denver
              </p>
              <a
                href="tel:+17204632319"
                className="inline-block bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold text-lg hover:bg-blue-700 transition-colors mb-6"
              >
                Call (720) 463-2319
              </a>
              <p className="text-gray-600">or submit your details online:</p>
            </div>
            <InquiryForm buttonText="Get Free Quotes" service="Siding & Gutters" />
          </div>
        </div>
      </div>
    );
  } catch (error) {
    console.error('Error rendering SidingGuttersPage:', error);
    // You can also return a custom error page or message here
    return <div>Error loading page</div>;
  }
}
