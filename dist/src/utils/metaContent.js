export function generateMetaContent(keyword, location, count = 10) {
    const title = `The ${count} Best ${keyword} in ${location.location}, ${location.county}`;
    const description = `Compare the top ${count} ${keyword.toLowerCase()} in ${location.location}, ${location.county}. View detailed profiles, customer reviews, and contact information.`;
    return {
        title,
        description,
        openGraph: {
...
export function generateStructuredData(keyword, location, count = 10) {
    return {
